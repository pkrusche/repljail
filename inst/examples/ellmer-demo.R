...
if (result$success) {
  ...
  # Execute some R code
  exec_result <- replr_execute_code(session_id, "x <- 10; y <- 20; x + y")
  if (exec_result$success) {
    cat("Execution output:", exec_result$data$output, "\n")
  }
  
  # Execute code with output
  result2 <- replr_execute_code(session_id, "summary(mtcars)")
  if (result2$success) {
    cat("Summary output:\n")
...
analysis1 <- replr_create_repl_session("data_analysis")
...
if (analysis1$success && analysis2$success) {
  ...
  replr_execute_code("data_analysis", "
    data <- mtcars
    summary_stats <- summary(data$mpg)
    data_type <- 'car_data'
  ")
  
  replr_execute_code("model_building", "
    data <- iris
  ...
  result1 <- replr_execute_code("data_analysis", "data_type")
  result2 <- replr_execute_code("model_building", "data_type")
...
if (error_demo$success) {
  ...
  error_result <- replr_execute_code(session_id, "stop('This is an error')")
  ...
  recovery_result <- replr_execute_code(session_id, "2 + 2")
...
for (id in sessions) {
  result <- replr_create_repl_session(id)
  if (result$success) {
...
# List all sessions
all_sessions <- replr_list_sessions()
cat("Total active sessions:", all_sessions$data$count, "\n")

# Get info about a specific session
info <- replr_get_session_info("session_b")
if (info$success) {
  cat("Session B info:\n")
...
# Clean up all sessions
cleanup <- replr_stop_all_sessions()
