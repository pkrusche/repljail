  ...
  tryCatch(
    {
      ...
      expect_true(!is.null(info$socket_path), "Native worker should use IPC socket")
  ...
  tryCatch(
    {
      # Check that session has socket_path (IPC mode)
      info <- session$get_info()
      expect_true(!is.null(info$socket_path), "Firejail worker should use IPC socket")
  ...
  tryCatch(
    {
      ...
      info <- session$get_info()
      expect_true(is.null(info$socket_path), "Docker worker should NOT use IPC socket")
      expect_true(!is.null(info$port) && info$port > 0, "Docker worker should use TCP port")
  ...
  tryCatch(
    {
      ...
      expect_true(file.exists(socket_path), "Socket file should be created by worker")
    },
    finally = {
      # Clean up
      if (proc$is_alive()) {
