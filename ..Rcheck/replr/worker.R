...
if (length(args) < 1 || length(args) > 2) {
  cat("Usage: Rscript worker.R <port> [--debug]\n", file = stderr())
...
debug_enabled <- length(args) >= 2 && args[2] == "--debug"
if (debug_enabled) {
...
worker_debug_log <- function(...) {
  if (is_debug_enabled()) {
    msg <- paste0("[", Sys.time(), "] ", ...)
    cat(cli::style_dim(cli::col_blue("\u2139")), " ", msg, "\n", file = stderr(), sep = "")
  }
}
...
worker_debug_success <- function(...) {
  if (is_debug_enabled()) {
    msg <- paste0("[", Sys.time(), "] ", ...)
    cat(cli::style_bold(cli::col_green("\u2714")), " ", msg, "\n", file = stderr(), sep = "")
  }
}
...
worker_debug_warn <- function(...) {
  if (is_debug_enabled()) {
    msg <- paste0("[", Sys.time(), "] ", ...)
    cat(cli::style_bold(cli::col_yellow("\u26a0")), " ", msg, "\n", file = stderr(), sep = "")
  }
}
...
worker_debug_error <- function(...) {
  if (is_debug_enabled()) {
    msg <- paste0("[", Sys.time(), "] ", ...)
    cat(cli::style_bold(cli::col_red("\u2716")), " ", msg, "\n", file = stderr(), sep = "")
  }
}
...
tryCatch({
  ...
  while (TRUE) {
    ...
    worker_debug_log("Sending response: ", paste0(deparse(response), collapse = ";"))
    # Send response back
